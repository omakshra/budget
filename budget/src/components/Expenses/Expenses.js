...
function Expenses() {
  ...
  const [startMonth, setStartMonth] = useState(""); // Start month (YYYY-MM)
  const [endMonth, setEndMonth] = useState(""); // End month (YYYY-MM)  
  useEffect(() => {
  ...
  const filteredMonthlyExpenses = useMemo(() => {
    ...
    return expenses.filter((expense) => {
      const expenseMonth = expense.date.slice(0, 7); // Extract "YYYY-MM"
  ...
  const chartData = {
    labels: filteredMonthlyExpenses.map((exp) => exp.date),
    datasets: [
      {
        label: "Expenses Between Selected Months",
        data: filteredMonthlyExpenses.map((exp) => exp.amount),
